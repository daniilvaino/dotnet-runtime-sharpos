...
Write-Output ""
Write-Output "extern const NativeStringResourceTable $TableName;"
Write-Output "const NativeStringResource $ArrayName[] = {"

# bsearch() in resourcestring.cpp requires entries sorted by id.
# Picked up from upstream PR #122405 (sort mscorrc.cpp when building
# on windows host) — keep this hunk identical to upstream so future
# merge doesn't conflict.
...
Write-Output "};"
Write-Output ""
Write-Output "#ifdef _MSC_VER"
Write-Output "const NativeStringResourceTable $TableName = { $($ResourceArray.Count), $ArrayName };"
Write-Output "#else"
Write-Output "const NativeStringResourceTable $TableName __attribute__((visibility(`"default`"))) = { $($ResourceArray.Count), $ArrayName };"
Write-Output "#endif"
