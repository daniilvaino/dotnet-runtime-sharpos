...
# SharpOS cross-host: добавлен const — как в processrc.ps1. При виндовом ABI
# const попадает в имя символа (@@B против @@A), и без него таблица,
# созданная этим скриптом, не сходится с объявлением у потребителя:
# "undefined symbol: nativeStringResourceTable_mscorrc". На unix-целях
# разницы нет — там const в имя переменной не кодируется.
printf "extern const NativeStringResourceTable %s;\n" "$tableName"
...
printf "};\n\n"
printf "const NativeStringResourceTable $tableName __attribute__((visibility(\"default\"))) = { %s, %s };\n" "${#resourceArray[@]}" "$arrayName"
